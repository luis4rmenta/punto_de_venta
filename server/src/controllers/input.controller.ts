...
export async function getInput(req: Request, res: Response): Promise<Response> {
    const inputId: number = parseInt(req.params.inputId)
    try {
        const resp = await getRow('entrada', 'entrada_id', inputId).then((resp: any) => resp[0]);
        return res.json(resp);
    } catch (error) {
        console.error(error);
        return res.json({ message: 'Error' });
...
export async function updateInput(req: Request, res: Response): Promise<Response> {
    try {
        ...
        const resp: MySQLUpdateResponse = await updateRow('entrada', 'entrada_id', inputId, inputUpdated).then((resp: any) => resp[0]);
...
export async function newInput(req: Request, res: Response): Promise<Response> {
    try {
        ...
        VALUES (?, ?, ?, ?, ?, ?);`,
            [newInput.fecha, newInput.total, newInput.estado_id,
            newInput.empleado_id, newInput.proveedor_id]).then((resp: any) => resp[0]);
...
export async function deleteInput(req: Request, res: Response) {
    try {
        const inputId: number = parseInt(req.params.inputId);
        const resp: MySQLDeletedResponse = await deleteRow('entrada', 'entrada_id', inputId);
